...
namespace UploadGL.Helpers
{
	public static class StringExtensions
	{
		...
		/// <summary>Regla way to create a HTML attribute with a leading space.</summary>
		...
		public static string Sha(this string value, Encoding encoding)
		{
			if (!string.IsNullOrWhiteSpace(value))
			{
				...
				{
					byte[] data = encoding.GetBytes(value);
					return sha1.ComputeHash(data).ToString();
				}
			}
		...
		public static string ReplaceNativeDigits(this string value, IFormatProvider provider = null)
		{
			...
			var result = rg.Replace(value, m => nativeDigits[Convert.ToInt32(m.Value)]);
			return result;
		}
